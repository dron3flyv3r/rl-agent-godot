...
[GlobalClass]
public partial class RLAgent3D : Node3D, IRLAgent
{
    ...
    /// <summary>Reward amount consumed in the most recent ConsumePendingReward call.</summary>
    ...
    /// <summary>Called each physics step. Override to compute rewards and call EndEpisode() when done.</summary>
    ...
    /// <summary>Returns true if EndEpisode() has been called and the done signal is pending.</summary>
    ...
    float[] IRLAgent.CollectObservationArray()
    {
        ...
        {
            GD.PushError(
                $"[RLAgent3D] Agent '{Name}' changed observation size from {_validatedObservationSize.Value} to {observation.Length}. " +
