...
public sealed class PpoInferencePolicy : IInferencePolicy
{
    ...
    public PpoInferencePolicy(int observationSize, int actionCount, RLNetworkConfig config)
    {
        if (actionCount <= 0)
        {
        ...
        _network = new PolicyValueNetwork(observationSize, actionCount, config);
    }
